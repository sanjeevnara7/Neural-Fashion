...
class BasicModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.MODEL_SEQ_LEN, bdash).zero_() for _ in range(group_size)]
        ...
        for t in range(self.MODEL_SEQ_LEN + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.MODEL_SEQ_LEN + divm - 1:
                    # add diversity
                    ...
                    for vix in range(bdash):
                        if beam_seq_table[divm][t-divm,vix] == 0 or t == self.MODEL_SEQ_LEN + divm - 1:
                            final_beam = {
